  ...
  module Compatibility
    ...
    class JsCompatibilityChecker
      ...
      def check_single_address(address)
        ...
        @results << { address: address, ruby_result: ruby_result, js_result: js_result, differences: differences, compatible: differences.empty? }
      end
      ...
      def find_differences(ruby_result, js_result)
        ...
        %w[pref city town addr level lat lng].each do |key|
          ...
          next unless ruby_value != js_value

          differences[key] = { ruby: ruby_value, javascript: js_value }
        end
      ...
      def export_to_csv(filename)
        CSV.open(filename, 'w') do |csv|
          csv << %w[住所 互換性 差異フィールド Ruby結果 JS結果]
          ...
          @results.each do |result|
            if result[:compatible]
              ...
              result[:differences].each do |key, diff|
                csv << [result[:address], '×', key, diff[:ruby], diff[:javascript]]
              end
            end
          end
        end
