...
::RSpec.describe(::MatchCloseToHelper) do
  def build_address(pref: nil, city: nil, town: nil, other: '', point: nil, level: 0)
    ...
    single_city = city && ::JapaneseAddressParser::Data::SingleCity.from_json('code' => 1, 'city' => city, 'city_k' => 'X', 'city_r' => 'X', 'point' => [139.0, 35.0])
    machi_aza = town && { machiaza_id: '1', oaza_cho: town, chome: nil, chome_n: nil, koaza: nil, point: nil }
    metadata = ::JapaneseAddressParser::NormalizeResultMetadata.new(input: 'x', prefecture:, city: single_city, machi_aza:, chiban: nil, rsdt: nil)
    result = ::JapaneseAddressParser::NormalizeResult.new(pref:, city:, town:, addr: nil, other:, point:, level:, metadata:)
    ::JapaneseAddressParser::Address.from_normalize_result(result)
  end

  def point(lat, lng, level)
    ::JapaneseAddressParser::NormalizeResultPoint.new(lat:, lng:, level:)
  end
