...
::RSpec.describe(::JapaneseAddressParser::CacheRegexes, :upstream_port) do
  ...
  describe '.get_prefectures' do
    it 'fetches all 47 prefectures as a PrefectureApi' do
      expect(api).to(be_a(::JapaneseAddressParser::Data::PrefectureApi))
  ...
  describe '.get_towns' do
    it 'fetches the town list as a MachiAzaApi' do
      ...
      expect(towns).to(be_a(::JapaneseAddressParser::Data::MachiAzaApi))
  ...
  describe '.cache' do
    it 'is an LruRedux cache that evicts at config.cache_size' do
      ::JapaneseAddressParser.instance_variable_set(:@config, nil)
      ::JapaneseAddressParser.configure { |c| c.cache_size = 2 }
