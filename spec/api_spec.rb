...
# 公開エントリ JapaneseAddressParser.call / JapaneseAddressParser.call!（M6）。
...
::RSpec.describe(::JapaneseAddressParser) do
  describe '.call', :upstream_port do
    ...
    it 'returns a rich Address with nested value objects' do
      ...
      expect(result).to(be_a(::JapaneseAddressParser::Address))
  ...
  describe 'fetch-failure contract (stubbed I/O boundary)' do
    ...
    # JapaneseAddressParser.call! が「fetch 失敗」として握る代表例外（lib/japanese_address_parser/v4.rb の rescue 一覧に対応）。
    ...
    [::SocketError, ::Errno::ECONNREFUSED, ::Net::OpenTimeout, ::Net::ReadTimeout, ::JSON::ParserError].each do |error_class|
      context "when the fetch raises #{error_class}" do
        before { allow(::JapaneseAddressParser::Normalize).to(receive(:call).and_raise(error_class)) }
    ...
    context 'when the fetch raises a non-fetch error (programmer/config error)' do
      ...
      before { allow(::JapaneseAddressParser::Normalize).to(receive(:call).and_raise(::ArgumentError, 'Unknown URL schema: ftp:')) }
