...
6. **アンカーは意味で訳す**: JS の `^`/`$`（`m` フラグ無し）は文字列の先頭/末尾を指すが、Ruby の `^`/`$` は行頭/行末を指す。文字列先頭/末尾の意味を保つため **`^`→`\A`、`$`→`\z` に翻訳**する（§3-1「逐語コピー」の例外。挙動の忠実性を §3-4 として優先する）。可能なら改行を含む入力で分岐を spec に固定する。初出: `patch_addr`（M2）。M5（`normalize.ts`）で多用される。
